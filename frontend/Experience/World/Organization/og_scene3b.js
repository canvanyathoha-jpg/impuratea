...
export default class OrganizationScene3B {
            ...
            setTimeout(() => {
                try {
                    ...
                    if (this.enableAdvancedAtmosphere) {
                        ...
                        console.log("[OrgScene3B] Skipping advanced atmospheric effects for low-end device");
                    }

                    if (this.enablePostProcessing) {
                        this.setupPostProcessing();
                    } else {
                        console.log("[OrgScene3B] Skipping post-processing composer for low-end device");
                        this.composer = null;
                    }
    ...
    createNPC() {
        ...
        if (!bosCaffeModel) {
            console.error("[OrgScene3B] Bos Caffe model not found!");
            return;
        }
    ...
    setupNPCAnimations() {
        ...
        if (!this.resources.items.bos_caffe || !this.resources.items.bos_caffe.animations) {
            console.warn("[OrgScene3B] Bos Caffe model has no animations, NPC will be static");
            return;
        }
        ...
        if (this.npcAnimations.length === 0) {
            console.warn("[OrgScene3B] No animations found for bos_caffe model");
            return;
        }
        ...
        if (idleAnimation) {
            ...
            console.warn("[OrgScene3B] Could not find any animation to play");
        }
    }
    ...
    setupMobileLighting() {
        console.log("[OrgScene3B] Applying lightweight lighting for low-end profile...");
    ...
    setupPostProcessing() {
        if (!this.enablePostProcessing) {
            console.log("[OrgScene3B] Post-processing skipped for current performance profile");
            this.composer = null;
            return;
        }
    ...
    setupAtmosphericEffects() {
        if (!this.enableAdvancedAtmosphere) {
            console.log("[OrgScene3B] Advanced atmospheric effects disabled for this device");
    ...
    handleChoice(choiceId) {
        ...
        if (this.experience && this.experience.scoreManager) {
            ...
            console.log(`[OrgScene3B] ScoreManager total score: ${this.experience.scoreManager.getScore()}%`);
        }
