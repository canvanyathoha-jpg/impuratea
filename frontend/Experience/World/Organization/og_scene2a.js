...
export default class OrganizationScene2A {
                    ...
                    setTimeout(() => {
                        try {
                            ...
                            if (this.enableAdvancedAtmosphere) {
                                ...
                                console.log("[OrgScene2A] Skipping advanced atmospheric effects for low-end device");
                            }

                            if (this.enablePostProcessing) {
                                this.setupPostProcessing();
                            } else {
                                console.log("[OrgScene2A] Skipping post-processing composer for low-end device");
                                this.composer = null;
                            }
    ...
    setupMobileLighting() {
        console.log("[OrgScene2A] Applying lightweight lighting for low-end device...");
    ...
    setupProfessionalLighting() {
        ...
        if (this.experience.renderer && this.experience.renderer.instance) {
            ...
            this.experience.renderer.instance.toneMappingExposure = allowShadows ? 0.6 : 1.1;
        }
        ...
        this.lights.npcSpot = new THREE.SpotLight(0xffe5e5, allowShadows ? 3.0 : 1.2);
    ...
    setupPostProcessing() {
        if (!this.enablePostProcessing) {
            console.log("[OrgScene2A] Post-processing skipped due to performance profile");
            this.composer = null;
            return;
        }
    ...
    setupAtmosphericEffects() {
        if (!this.enableAdvancedAtmosphere) {
            console.log("[OrgScene2A] Advanced atmospheric effects disabled for this device");
            // Apply a lightweight fog to preserve mood without particles
            this.scene.fog = new THREE.Fog(0x12121c, 40, 160);
            return;
        }
