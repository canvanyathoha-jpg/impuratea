...
export default class OrganizationScene3A {
    ...
    initWithOpening() {
        console.log("[OrgScene3A] Initializing with opening story...");
        ...
        const waitForResources = () => {
            if (this.resources.isReady) {
                console.log("[OrgScene3A] ✅ Resources ready, starting scene load...");
                this.loadSceneAsync().then(() => {
                    ...
                    this.openingStory.show().then(() => {
                        console.log("[OrgScene3A] Opening story dismissed");
                ...
                console.log("[OrgScene3A] ⏳ Waiting for resources to be ready...");
                this.resources.once('ready', () => {
                    console.log("[OrgScene3A] ✅ Resources ready event fired!");
                    waitForResources();
                });
            }
        };
    ...
    createSimpleSpeechBubble() {
        ...
        const dialogue = languageManager.translate(ORG_TEXTS.scene3a.dialogue);
        ...
        if (userCanAutoPlay) {
            ...
            console.warn("[OrgScene3A] Skipping auto speech (browser requires user interaction). Click the bubble to play audio.");
        }
    }
    ...
    createSpeechTextTexture() {
        ...
        const speakerName = languageManager.translate(ORG_TEXTS.scene3a.speaker) + ':';
        ...
        const dialogue = languageManager.getLanguage() === 'id' ?
            ...
            "I can be your vendor,\nbut with a special system:\nsignature without official reports.\nBonus funds for you all.";
    ...
    setupProfessionalLighting() {
        ...
        console.log("[OrgScene3A] Cinematic dark lighting system setup complete");
    }
    ...
    setupPostProcessing() {
        ...
        if (!renderer || !camera) {
            console.warn("[OrgScene3A] Renderer or camera not available for post-processing");
            return;
        }
        ...
        console.log("[OrgScene3A] Advanced cinematic post-processing setup complete");
    ...
    setupAtmosphericEffects() {
        ...
        console.log("[OrgScene3A] Atmospheric effects setup complete");
    }

    enhanceMaterials() {
        console.log("[OrgScene3A] Enhancing materials with PBR...");

        if (!this.caffeModel) {
            console.warn("[OrgScene3A] Caffe model not available for material enhancement");
            return;
        }
        ...
        console.log("[OrgScene3A] Materials enhanced with PBR properties");
    }
        ...
        screenBubble.innerHTML = `
            ...
            ">
                <div style="
                    ...
                    letter-spacing: 1px;
                ">
                    ${languageManager.translate(ORG_TEXTS.scene3a.speaker)}:
                </div>
                <div style="
                    font-size: 17px;
                    color: rgba(255, 255, 255, 0.9);
                ...
                ">
                    ${languageManager.translate(ORG_TEXTS.scene3a.dialogue)}
                </div>
                <div style="
                    text-align: center;
                    margin-top: 25px;
    ...
    onMouseClick(event) {
        ...
        if (intersects.length > 0) {
            console.log("[OrgScene3A] Speech bubble clicked!");
    ...
    showChoices() {
        console.log("[OrgScene3A] Creating dark cinematic choice panel...");
                    ...
                    ">
                        ...
                        <div style="flex: 1; position: relative; z-index: 1;">
                            <div style="
                                ...
                                line-height: 1.4;
                                color: rgba(255,255,255,0.95);
                            ">${languageManager.translate(ORG_TEXTS.scene3a.choices.a)}</div>
                        </div>
                    </button>
                    ...
                    ">
                        ...
                        <div style="flex: 1; position: relative; z-index: 1;">
                            <div style="
                                ...
                                line-height: 1.4;
                                color: rgba(255,255,255,0.95);
                            ">${languageManager.translate(ORG_TEXTS.scene3a.choices.b)}</div>
                        </div>
                    </button>
                </div>
            </div>
        ...
        message.innerHTML = `
            <div style="background: linear-gradient(135deg, rgba(255,152,0,0.95), rgba(255,193,7,0.95)); border: 3px solid rgba(255,255,255,0.5); border-radius: 20px; padding: 30px; max-width: 600px; box-shadow: 0 8px 32px rgba(0,0,0,0.5); text-align: center;">
                ...
                <div style="font-size: 18px; color: #fff; line-height: 1.6; font-style: italic;">
                    "${languageManager.translate(ORG_TEXTS.scene3a.supplementMessage)}"
                </div>
            </div>
        `;
