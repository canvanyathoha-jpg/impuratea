...
export default class OrganizationScene4B {
    ...
    initWithOpening() {
        console.log("[OrgScene4B] Initializing with opening story...");
        ...
        const waitForResources = () => {
            if (this.resources.isReady) {
                console.log("[OrgScene4B] ✅ Resources ready, starting scene load...");
                this.loadSceneAsync().then(() => {
                    ...
                    this.openingStory.show().then(() => {
                        console.log("[OrgScene4B] Opening story dismissed");
                ...
                console.log("[OrgScene4B] ⏳ Waiting for resources to be ready...");
                this.resources.once('ready', () => {
                    console.log("[OrgScene4B] ✅ Resources ready event fired!");
                    waitForResources();
                });
            }
        };
                    ...
                    setTimeout(() => {
                        try {
                            ...
                            if (this.enableAdvancedAtmosphere) {
                                ...
                                console.log("[OrgScene4B] Skipping advanced atmospheric effects for low-end device");
                            }

                            if (this.enablePostProcessing) {
                                this.setupPostProcessing();
                            } else {
                                console.log("[OrgScene4B] Skipping post-processing composer for low-end device");
                                this.composer = null;
                            }
    ...
    createSimpleSpeechBubble() {
        ...
        if (this.aiVoiceTimeout) {
            clearTimeout(this.aiVoiceTimeout);
                ...
                const dialogue = languageManager.translate(ORG_TEXTS.scene4b.dialogue);
        ...
        if (userCanAutoPlay) {
            ...
            console.warn("[OrgScene4B] Skipping auto speech (browser requires user interaction). Click the bubble to play audio.");
        }
    }
    ...
    createSpeechTextTexture() {
        ...
        const speakerName = languageManager.translate(ORG_TEXTS.scene4b.speaker) + ':';
        ...
        const dialogue = languageManager.getLanguage() === 'id' ?
            "Ada beberapa perlengkapan\nyang tidak sesuai.\nSaya perlu laporan keuangan\nsementara untuk melihat kondisi dana." :
            "There are some equipment\nthat don't match.\nI need an interim financial report\nto review the fund status.";
    ...
    setupMobileLighting() {
        console.log("[OrgScene4B] Applying lightweight lighting for low-end profile...");
    ...
    setupProfessionalLighting() {
        console.log("[OrgScene4B] Setting up cinematic dark lighting system...");
        ...
        console.log("[OrgScene4B] Cinematic dark lighting system setup complete");
    }

    setupPostProcessing() {
        if (!this.enablePostProcessing) {
            console.log("[OrgScene4B] Post-processing skipped for current performance profile");
            this.composer = null;
            return;
        }
        console.log("[OrgScene4B] Setting up advanced cinematic post-processing...");
        ...
        if (!renderer || !camera) {
            console.warn("[OrgScene4B] Renderer or camera not available for post-processing");
            return;
        }
        ...
        console.log("[OrgScene4B] Advanced cinematic post-processing setup complete");
    ...
    setupAtmosphericEffects() {
        if (!this.enableAdvancedAtmosphere) {
            console.log("[OrgScene4B] Advanced atmospheric effects disabled for this device");
            this.scene.fog = new THREE.Fog(0x12121c, 45, 160);
            return;
        }
        console.log("[OrgScene4B] Setting up dark atmospheric effects...");
        ...
        console.log("[OrgScene4B] Atmospheric effects setup complete");
    }

    enhanceMaterials() {
        console.log("[OrgScene4B] Enhancing materials with PBR...");

        if (!this.ruangGuruModel) {
            console.warn("[OrgScene4B] RuangGuru model not available for material enhancement");
            return;
        }
        ...
        console.log("[OrgScene4B] Materials enhanced with PBR properties");
    }
        ...
        screenBubble.innerHTML = `
            ...
            ">
                <div style="
                    ...
                    letter-spacing: 1px;
                ">
                    ${languageManager.translate(ORG_TEXTS.scene4b.speaker)}:
                </div>
                <div style="
                    font-size: 17px;
                    color: rgba(255, 255, 255, 0.9);
                ...
                ">
                    ${languageManager.translate(ORG_TEXTS.scene4b.dialogue)}
                </div>
                <div style="
                    text-align: center;
                    margin-top: 25px;
    ...
    onMouseClick(event) {
        ...
        if (intersects.length > 0) {
            console.log("[OrgScene4B] Speech bubble clicked!");
    ...
    showChoices() {
        console.log("[OrgScene4B] Creating dark cinematic choice panel...");
            ...
            opacity: 0;
                        ...
                        <div style="flex: 1; position: relative; z-index: 1;">
                            <div style="
                                ...
                                line-height: 1.4;
                                color: rgba(255,255,255,0.95);
                            ">${languageManager.translate(ORG_TEXTS.scene4b.choices.a)}</div>
                        </div>
                    </button>
                    ...
                    ">
                        ...
                        <div style="flex: 1; position: relative; z-index: 1;">
                            <div style="
                                ...
                                line-height: 1.4;
                                color: rgba(255,255,255,0.95);
                            ">${languageManager.translate(ORG_TEXTS.scene4b.choices.b)}</div>
                        </div>
                    </button>
                </div>
            </div>
        ...
        message.innerHTML = `
            <div style="background: linear-gradient(135deg, rgba(255,152,0,0.95), rgba(255,193,7,0.95)); border: 3px solid rgba(255,255,255,0.5); border-radius: 20px; padding: 30px; max-width: 600px; box-shadow: 0 8px 32px rgba(0,0,0,0.5); text-align: center;">
                ...
                <div style="font-size: 18px; color: #fff; line-height: 1.6; font-style: italic;">
                    "${languageManager.translate(ORG_TEXTS.scene4b.supplementMessage)}"
                </div>
            </div>
        `;
    ...
    dispose() {
        ...
        console.log("[OrganizationScene4B] Disposed");
    }
}
