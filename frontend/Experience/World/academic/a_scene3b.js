...
export default class AcademicScene3B {
    ...
    createNPC() {
        ...
        console.log("[AcademicScene3B] Available animations:", this.npcAnimations.length);
        console.log("[AcademicScene3B] Animation names:", this.npcAnimations.map(clip => clip.name));
        ...
        if (idleClip) {
            try {
                ...
                console.log("[AcademicScene3B] Idle animation created:", idleClip.name);
                this.npcActions.idle.play();
            } catch (error) {
                console.warn("[AcademicScene3B] Failed to create/play idle animation:", error);
            }
        } else {
            console.warn("[AcademicScene3B] No idle animation found, NPC will be static");
        }
    ...
    createTeacherNPC() {
        ...
        console.log("[AcademicScene3B] Teacher animations count:", this.teacherAnimations.length);
        console.log("[AcademicScene3B] Teacher animation names:", this.teacherAnimations.map(clip => clip.name));
        ...
        if (this.teacherAnimations.length > 0) {
            ...
            if (idleClip) {
                try {
                    ...
                    console.warn("[AcademicScene3B] Failed to create/play teacher idle animation:", error);
                }
            }
        }
        ...
        if (this.storyStarted) {
            ...
            if (this.speechBubbleGroup) {
                try {
                    ...
                    console.warn("[AcademicScene3B] Error setting raycaster from camera:", error);
                }
            }
            return;
        }
    ...
    transitionToNextScene(sceneName) {
        ...
        if (this.experience.world && this.experience.world.switchSceneWithPosition) {
            ...
            console.log(`[AcademicScene3B] Switching to ${sceneName} at position:`, spawnPoint);
            this.experience.world.switchSceneWithPosition(sceneName, spawnPoint);
        } else {
            console.error("[AcademicScene3B] World.switchSceneWithPosition not available, falling back to reload");
