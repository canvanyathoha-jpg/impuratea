...
export default class AcademicScene4B {
    ...
    createTeacherNPC() {
        console.log("[AcademicScene4B] Creating Teacher NPC...");
        const teacherModel = this.resources.items.teacher;
        if (!teacherModel) {
            console.error("[AcademicScene4B] Teacher model not found!");
            return;
        }

        // Clone dengan scene.clone() untuk mempertahankan material
        this.teacherNPC = teacherModel.scene.clone();
        this.teacherNPC.position.set(0, 0, -30); // Guru menghadap ke kelas, Y = 0 untuk ground level
        ...
        console.log("[AcademicScene4B] Teacher NPC created.");
    }
    ...
    create3DSpeechBubble(speaker, text, showScreenDialog = true) {
        ...
        // Posisikan speech bubble di samping NPC, setinggi kepala
    ...
    transitionToNextScene(sceneName) {
        ...
        if (this.experience.world && this.experience.world.switchSceneWithPosition) {
            ...
            console.log(`[AcademicScene4B] Switching to ${sceneName} at position:`, spawnPoint);
            this.experience.world.switchSceneWithPosition(sceneName, spawnPoint);
        } else {
            console.error("[AcademicScene4B] World.switchSceneWithPosition not available, falling back to reload");
