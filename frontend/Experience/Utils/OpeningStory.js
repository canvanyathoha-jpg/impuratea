export default class OpeningStory {
    ...
    show() {
        ...
        const title = this.sceneData.title || "Scene 1: Organisasi Mahasiswa";
        const story = this.sceneData.story || [
            "Kamu adalah seorang mahasiswa yang baru saja bergabung dengan organisasi mahasiswa di kampus. Sebagai bendahara junior, kamu bertanggung jawab mengelola dana organisasi.",
        ...
        this.overlay.innerHTML = `
            <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 10000; display: flex; flex-direction: column; justify-content: center; align-items: center; color: white; font-family: 'Arial', sans-serif; cursor: pointer;">
                <div style="max-width: 800px; text-align: center; padding: 40px; opacity: 0; transform: translateY(50px); transition: all 1s ease; background: rgba(0,0,0,0.9); border-radius: 20px; border: 2px solid rgba(255,255,255,0.3); box-shadow: 0 8px 32px rgba(0,0,0,0.5);">
                    <h1 style="font-size: 48px; margin-bottom: 30px; color: #ff6b6b; text-shadow: 0 0 20px rgba(255,107,107,0.8); font-weight: bold;">
                        ${title}
                    </h1>
        ...
        setTimeout(() => {
            ...
            if (content) {
                ...
                console.log(`[OpeningStory] Opening story faded in for: ${this.sceneData.title}`);
...
export const SCENE_DATA = {
    og_scene1: {
        title: "Scene 1: Organisasi Mahasiswa",
        story: [
            "Kamu adalah seorang mahasiswa yang baru saja bergabung dengan organisasi mahasiswa di kampus. Sebagai bendahara junior, kamu bertanggung jawab mengelola dana organisasi.",
            "Hari ini, Senior Bendahara memanggilmu untuk membicarakan sesuatu yang penting...",
        ],
